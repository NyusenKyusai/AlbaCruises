...
$userDetails = $db->findAccountDetailsByUsername($conn, $username);
...
-->
	...
	<div class="main">
		...
		<form action="../private/editDetailsHandle.php" method="post" onSubmit="return checkForm(this)">
			Username: <br><input type="text" name="username" value="<?php echo $userDetails['username']; ?>" required><br>
			Password: <br><input type="text" name="password" required><br>
			Date of Birth: <br><input type="date" name="dateBirth" value="<?php echo $userDetails['dateBirth']; ?>" required><br>
			Email: <br><input type="email" name="email" value="<?php echo $userDetails['email']; ?>" required><br>
			<br><input type="submit" value="Save Details" id="button"><br>
		</form>
	</div>
	<!-- InstanceEndEditable -->
	<footer>
